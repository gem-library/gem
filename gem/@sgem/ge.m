...
function result = ge(this, varargin)
    ...
    if length(varargin) ~= 1
        error('Wrong number of arguments in sgem::ge');
    end
    ...
    if (~isequal(size1, size2)) && (prod(size1) ~= 1) && (prod(size2) ~= 1)
        error('Incompatible size for ge comparison');
    end
    ...
    if isequal(class(this), 'gem') && isequal(class(varargin{1}), 'gem')
        ...
        result = logical(gem_mex('ge', objId1, objId2));
    else
        objId1 = this.objectIdentifier;
        objId2 = varargin{1}.objectIdentifier;
        result = logical(sgem_mex('ge', objId1, objId2));
    end

    % For matlab, ge when one object is sparse produces a sparse result

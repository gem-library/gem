% ne - inequality test
function result = ne(this, varargin)
    ...
    if length(varargin) ~= 1
        error('Wrong number of arguments in sgem::ne');
    end
    ...
    if (~isequal(size1, size2)) && (prod(size1) ~= 1) && (prod(size2) ~= 1)
        error('Incompatible size for ne comparison');
    end
    ...
    if isequal(class(this), 'gem') && isequal(class(varargin{1}), 'gem')
        ...
        result = logical(gem_mex('ne', objId1, objId2));
    else
        objId1 = this.objectIdentifier;
        objId2 = varargin{1}.objectIdentifier;
        result = logical(sgem_mex('ne', objId1, objId2));
    end
